...
### 2026-06-03 — ARCHITECTURE — buildStepState (snapshot stepwise durable) throw sur NaN d'une donnée d'agent → crash du run, Tier 3 sauté
- **Contexte** : 1er test PROD du Deep Dive stepwise durable (flag DEEP_DIVE_STEPWISE=1) sur avekapeti. Tier 0 a rebondi (replay après >300s), mais le run a FAILED à ~29 min en Tier 1 phase D ; remboursé ; Tier 3 jamais exécuté.
- **Erreur (exception persistée dans analysis.summary)** : `[buildStepState] nombre non-fini (NaN/Infinity) en $.allResults.gtm-analyst.data.meta.confidenceLevel`. `normalizeToWire` (bridge live→wire du snapshot, full-analysis-step-state-bridge.ts) REJETAIT par throw tout nombre non-fini. gtm-analyst (SUCCEEDED) a émis confidenceLevel=NaN → throw au snapshot tier1-d-agents → catch global du driver v3 → failFullAnalysis → FAILED, et tout Tier 3 (APRÈS dans le graphe) sauté.
- **Cause racine** : (1) régression STEPWISE-ONLY — le chemin OFF persiste allResults via JSON.parse(JSON.stringify) qui convertit NaN→null silencieusement ; buildStepState (strict, gate Codex b-2) throwait. (2) AMONT : ~13 agents partageaient `data.meta?.confidenceLevel == null` (ne capture que null/undefined) → une valeur non-numérique passait à Math.max(0,x) → NaN. (3) couplages annexes : un GET pollé (analyses route) muttait un run RUNNING>3h en FAILED (un refresh pouvait tuer le run) ; compteur completedAgents figé entre frontières de tier.
- **Solution validée (branche fix/deepdive-stepwise-resilience, 4 commits gatés Codex APPROVE)** : Bug 1 — normalizeToWire NaN/Infinity→null (parité OFF) + warn agrégé ; strict conservé pour Map/Set/classe/function ; asserts inchangés (scalaire structurel reste fail-loud). Bug 1b — helper partagé clampConfidenceLevel (fini-uniquement) sur les 13 agents. Bug 2 — suppression auto-expire GET 3h (reaper durable 20 min suffit). Bug 3 — flush completedAgents à chaque snapshot via updateMany conditionnel atomique dans la transaction du checkpoint.
- **Lesson** : un sérialiseur STRICT qui throw sur de la DONNÉE D'AGENT (LLM, malformable par nature) ne doit jamais être sur le chemin critique d'un run durable — il transforme un champ bénin en kill total. Convertir (JSON-faithful) la donnée d'agent ; garder strict UNIQUEMENT les invariants structurels. Et : un GET ne mute jamais un run ; seul le watchdog durable terminalise.

### 2026-06-03 — DURABILITÉ — Split d'un graphe stepwise durable EN PLACE alors que le flag est ON → mismatch de step IDs au replay cross-deploy
- **Fichier(s)** : `src/agents/orchestrator/index.ts` (runFullAnalysisStepwiseV3, dispatch runFullAnalysis), `src/agents/orchestrator/full-analysis-driver.ts` (STEPWISE_GRAPH_VERSION).
- **Erreur** : en implémentant le split tier0-thesis (suivi post-mortem #2), j'ai d'abord modifié le graphe v3 EN PLACE — renommé le step `tier0-thesis` en `tier0-pre-context` + `tier0-thesis-extractor` — SANS bumper STEPWISE_GRAPH_VERSION. Or `DEEP_DIVE_STEPWISE` est désormais ON en prod (avekapeti COMPLETED) → des runs `graphVersion=3` peuvent être en vol au moment d'un deploy. Au replay Inngest (re-déroule le handler depuis le début), le nouveau code appelle `stepRunner.run("tier0-pre-context")` alors que le run en vol a mémoïsé `stepRunner.run("tier0-thesis")` → ID non mémoïsé → RE-EXÉCUTION de doc-extractor/deck/context (coût + drift `evidenceToday=new Date()`) et, pire, le snapshot `tier0-pre-context` (createdAt=now) devient le « latest » lu par `ensureRehydrated` → RÉGRESSION (un run rendu à Tier1 repart de Tier0).
- **Cause racine** : les splits d-4..d-7 avaient raffiné le graphe v3 EN PLACE sans bump — légitimement, car le flag était OFF (aucun run v3 en vol, invariant explicité dans le doc-comment de STEPWISE_GRAPH_VERSION). Le flag est passé ON → l'invariant « pas de run en vol » ne tient plus. J'ai reconduit le pattern « raffiner en place » sans réévaluer sa précondition.
- **Solution validée** : bump `STEPWISE_GRAPH_VERSION` 3→4 ; le dispatch route `graphVersion===3` → graphe FROZEN (param `tier0Split=false`, step `tier0-thesis` inchangé) et `===4` → split (`tier0Split=true`) ; version STICKY (stampée dans l'event, immuable par run) → un run v3 en vol reprend TOUJOURS sur son graphe figé (mêmes step IDs). Driver PARAMÉTRÉ (un seul `runFullAnalysisStepwiseV3`, bloc tier0 branché) plutôt que dupliqué. Test `E1-v3-frozen` verrouille `fake.executedIds` contient `"tier0-thesis"` et PAS les IDs v4. tsc 0, vitest 4275/2skip, gate Codex design+impl APPROVE.

...
// Chantier P5-b — note de deal PURGÉE du type : overallScore / confidence / dimensionScores / scoreBreakdown / comparativeRanking retirés (production retirée en P4, restitution scoreless en P3). Les snapshots historiques qui les portent encore sont lus défensivement (cast Record), jamais via ce type.
export interface SynthesisDealScorerData { verdict: Tier3Orientation; investmentRecommendation: { action: Tier3Orientation; rationale: string; conditions?: string[]; suggestedTerms?: string }; keyStrengths: string[]; keyWeaknesses: string[]; criticalRisks: string[]; signalContribution: Tier3SignalContribution }
...
export interface MemoGeneratorData { executiveSummary: { oneLiner: string; recommendation: "very_favorable" | "favorable" | "contrasted" | "vigilance" | "alert_dominant"; keyPoints: string[] }; signalProfile: Tier3SignalContribution; criticalRisks: CriticalRiskRef[]; companyOverview: { description: string; problem: string; solution: string; businessModel: string; traction: string }; investmentHighlights: { highlight: string; evidence: string; dbComparable?: string; source?: string }[]; keyRisks: { risk: string; severity?: "CRITICAL" | "HIGH" | "MEDIUM"; category?: string; mitigation: string; residualRisk: "low" | "medium" | "high"; source?: string }[]; financialSummary: { currentMetrics: Record<string, string | number>; projections: string; valuationAssessment: string }; teamAssessment: string; marketOpportunity: string; competitiveLandscape: string; dealTerms: { valuation: string; roundSize: string; keyTerms: string[]; negotiationPoints: string[] }; dueDiligenceFindings: { completed: string[]; outstanding: string[]; redFlags: string[] }; investmentThesis: string; nextSteps: string[]; appendix: { financialModel?: string; comparableDeals?: string[]; referencesChecked?: string[] } }

...
function transformOutput(raw: CybersecurityExpertOutput): SectorExpertData {
  return {
...
function buildExtendedData(raw: CybersecurityExpertOutput): Partial<ExtendedSectorData> {
  return {
    subSector: {
  ...
  async run(context: EnrichedAgentContext): Promise<SectorExpertResult> {
    ...
    try {
      ...
      try {
        ...
        parsedOutput = CyberOutputSchema.parse(JSON.parse(jsonMatch[0]));
      } catch (parseError) {

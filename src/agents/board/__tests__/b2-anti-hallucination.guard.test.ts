/**
 * Phase B slice B2 — Source guard Board anti-hallucination cleanup.
 *
 * Vérifie mécaniquement que `src/agents/board/board-member.ts` :
 *
 ...
 *    `src/agents/orchestration/prompts/anti-hallucination.ts` (single
 *    source of truth — déjà utilisé par `board-orchestrator.ts` et
 *    `tier0/fact-extractor.ts`).
 *
 * Scope strict B2 (cf. brief utilisateur) :
 * - `src/agents/board/board-member.ts` UNIQUEMENT.
 * - `board-orchestrator.ts` n'est PAS touché (déjà migré).
 * - `context-compressor.ts`, `types.ts`, `index.ts` n'ont pas de bloc
 *   anti-hallucination — hors scope.
 *
 * Pattern dérivé du guard A9-reste (cross-target Phase A).
 */
...
const BOARD_MEMBER_PATH = "src/agents/board/board-member.ts";
 ...
 * Sert à filtrer les vrais positifs : un seuil numérique en contexte
 * business reste autorisé, un seuil en contexte d'auto-confidence est banni.
 */
...
describe("Phase B B2 — Source guard board-member.ts (anti-hallucination)", () => {
  const source = loadFile(BOARD_MEMBER_PATH);
  ...
  describe("2. Seuils numériques contextuels (faux positifs business autorisés)", () => {
    it("ne contient AUCUN seuil `50-90%` dans le contexte Structured Uncertainty", () => {
      ...
      if (violations.length > 0) {
        ...
        throw new Error(`Source guard B2 — seuil \`50-90%\` en contexte d'auto-confidence trouvé:\n${detail}`);
      }
    ...
    it("ne contient AUCUN seuil `<50%` dans le contexte Structured Uncertainty", () => {
      ...
      if (violations.length > 0) {
        const detail = violations.map((v) => `  L${v.lineNumber}: ${v.line}`).join("\n");
        throw new Error(`Source guard B2 — seuil \`<50%\` en contexte d'auto-confidence trouvé:\n${detail}`);
      }
      expect(violations).toEqual([]);
    });
    ...
    it("ne contient AUCUN seuil `>90%` dans le contexte Structured Uncertainty", () => {
      ...
      if (violations.length > 0) {
        const detail = violations.map((v) => `  L${v.lineNumber}: ${v.line}`).join("\n");
        throw new Error(`Source guard B2 — seuil \`>90%\` en contexte d'auto-confidence trouvé:\n${detail}`);
      }
      expect(violations).toEqual([]);
    });
  });

...
beforeEach(() => {
  ...
  const tx = {
    ...
    documentExtractionPage: { deleteMany: mocks.txPageDeleteMany },
    documentExtractionRun: { update: mocks.txRunUpdate },
    document: {
...
describe("completeDocumentExtractionRun — atomic documentFinalization", () => {
  it("updates the parent Document INSIDE the same transaction as the run terminal status", async () => {
    ...
    expect(mocks.txRunUpdate).toHaveBeenCalledTimes(1);
    ...
    const runOrder = mocks.txRunUpdate.mock.invocationCallOrder[0];
  ...
  it("P1: forces run status FAILED when the final corpus is empty, even if the manifest says ready_with_warnings", async () => {
    ...
    expect(mocks.txRunUpdate).toHaveBeenCalledTimes(1);
    const runUpdateData = mocks.txRunUpdate.mock.calls[0]?.[0]?.data;

...
export const ResolutionDialog = memo(function ResolutionDialog({
  ...
  const formKey = `${open ? "open" : "closed"}:${alertTitle}:${existingStatus ?? "RESOLVED"}:${existingJustification ?? ""}`;
...
const ResolutionDialogBody = memo(function ResolutionDialogBody({
  onOpenChange,
  alertTitle,
  alertSeverity,
  alertDescription,
  ...
  existingStatus,
  existingJustification,
}: Omit<ResolutionDialogProps, "open">) {

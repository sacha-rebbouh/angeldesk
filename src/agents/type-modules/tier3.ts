...
export interface ScenarioModelerFindings { scenarios: ScenarioV2[]; sensitivityAnalysis: SensitivityAnalysisV2[]; basedOnComparables: ScenarioComparable[]; breakEvenAnalysis: { monthsToBreakeven: number; breakEvenCalculation: string; requiredGrowthRate: number; growthRateSource: string; burnUntilBreakeven: number; burnCalculation: string; achievability: "ACHIEVABLE" | "CHALLENGING" | "UNLIKELY" | "UNKNOWN"; achievabilityRationale: string }; probabilityWeightedOutcome: { expectedMultiple: number; expectedMultipleCalculation: string; expectedIRR: number; expectedIRRCalculation: string; riskAdjustedAssessment: string }; mostLikelyScenario: "BASE" | "BULL" | "BEAR" | "CATASTROPHIC"; mostLikelyRationale: string }
